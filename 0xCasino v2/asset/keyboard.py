...
createGame = types.InlineKeyboardButton(
    text="🔥Создать игру", callback_data="createGame"
)
...
slotsBack = types.InlineKeyboardButton(text="Назад", callback_data="slotsBack")

menuUser = types.ReplyKeyboardMarkup(True, False)
menuUser.add("🎰 Слоты", "🃏 21 очко")
menuUser.add("🖥 Кабинет", "🏆 Турнир")
menuUser.add("ℹ️ Информация", "💵 Пополнить")
...
adminPanel.add("Управление топом дня")
adminPanel.add("Рассылка")
adminPanel.add("Назад")

topAdm = types.ReplyKeyboardMarkup(True, False)
topAdm.add("Вкл/выкл топ", "Пересоздать топ")
topAdm.add("Назад")
...
pravila.add(
    types.InlineKeyboardButton(
        text="🛡 Правила",
        url="https://telegra.ph/Pravila-bota-BlackJackJones-bot-11-20-2",
    )
)
...
qiwiAdm.add("Бонус к пополнению")
qiwiAdm.add("Бонус к пополнению FK")
qiwiAdm.add("<<<Назад")

qiwiAdmBns = types.ReplyKeyboardMarkup(True, False)
qiwiAdmBns.add("Включить/выключить", "Изменить дату окончания")
qiwiAdmBns.add("Изменить процент", "Изменить минимум")
qiwiAdmBns.add("<<<Назад")

fkAdmBns = types.ReplyKeyboardMarkup(True, False)
fkAdmBns.add("Включить/выключить FK", "Изменить дату окончания FK")
fkAdmBns.add("Изменить процент FK", "Изменить минимум FK")
fkAdmBns.add("<<<Назад")
...
def gameAll():
    ...
    for i in Games.select().where(Games.gamerTwo == 0):
        gameBtn1 = types.InlineKeyboardButton(
            text=f"💢 #Game_{i.id} | Сумма {i.balance}р", callback_data=f"gameCon_{i.id}"
        )
        game.add(gameBtn1)
...
def gameUser(user_id):
    ...
    for i in Games.select().where(Games.user_id == user_id):
        if i.gamerTwo == 0:
            gameBtn1 = types.InlineKeyboardButton(
                text=f"Сумма {i.balance}р | 💢Отмена", callback_data=f"gameDel_{i.id}"
            )
            game.add(gameBtn1)
    game.add(backBtn)
    return game
...
def keyFirst(id):
    ...
    btn2 = types.InlineKeyboardButton(
        text="Хватит, пусть играет", callback_data=f"firstGive_{id}"
    )
    gameStart.add(btn)
    gameStart.add(btn2)
    return gameStart
...
def keyTwo(id):
    ...
    btn2 = types.InlineKeyboardButton(
        text="Хватит, вскрываемся", callback_data=f"allIn_{id}"
    )
    gameStart.add(btn)
    gameStart.add(btn2)
    return gameStart
...
cabinetBal = types.InlineKeyboardButton(
    text="⚜Пополнить баланс", callback_data="addBalance"
)
cabinetGive = types.InlineKeyboardButton(
    text="⚜Вывод средств", callback_data="giveMoney"
)
referalSys = types.InlineKeyboardButton(
    text="👥Реферальная система", callback_data="refSys"
)
...
def informKeyboard():
    ...
    rulesGame = types.InlineKeyboardButton(
        text="📕Правила игры",
...
def top10gamers():
    gamers = (
        Global.select()
        .where(Global.username != "@None")
        .order_by(Global.countWin.desc(), Global.countBad.asc())
        .limit(10)
    )
    markup = types.InlineKeyboardMarkup()
    ...
    for i in gamers:
        btn = types.InlineKeyboardButton(
            text=f"ℹ{i.username[1:]} |🕹{i.countWin + i.countBad} Игры |🏆{i.countWin} Победы |☹{i.countBad} Проигрыши",
            url=f"t.me/{i.username[1:]}",
        )
        markup.add(btn)
    markup.add(backBtn2)
    return markup
...
def payment():
    ...
    btc = types.InlineKeyboardButton(
        text="⚜BTC | БАНКИР", callback_data="addBalanceBtc"
    )
    promo = types.InlineKeyboardButton(
        text="💵Промо-код💵", callback_data="promoActivate"
    )
    _free_kassa = types.InlineKeyboardButton(
        text="🔱Другие методы (Free-Kassa)", callback_data="addBalanceFreeKassa"
    )

    markup.add(qiwi, btc)
    markup.add(_free_kassa)
    markup.add(promo)

    return markup


def closeMessage(phone, output_id):
    markup = types.InlineKeyboardMarkup()
    btnSendMoney = types.InlineKeyboardButton(
        text="Выплатить", callback_data=f"send_{output_id}_{phone}"
    )
    btnBackMoney = types.InlineKeyboardButton(
        text="Вернуть деньги", callback_data=f"backMoney_{output_id}"
    )
...
def gameAdminDelete():
    ...
    for i in Games.select().where(Games.gamerTwo == 0):
        gameBtn1 = types.InlineKeyboardButton(
            text=f"Сумма {i.balance}р | 💢Отмена", callback_data=f"gameDelAdmin_{i.id}"
        )
        game.add(gameBtn1)
    game.add(backBtn)
    return game
...
def promoRemoute():
    markup = types.InlineKeyboardMarkup()
    addPromo = types.InlineKeyboardButton(
        text="Создать промо", callback_data="createPromo"
    )
    delPromo = types.InlineKeyboardButton(
        text="Удалить промо", callback_data="deletePromo"
    )
    activePromo = types.InlineKeyboardButton(
        text=f"Активные промо", callback_data="activePromo"
    )
