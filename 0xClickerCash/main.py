...
@dp.message_handler(content_types=["text"]) # Реакция на текст#Бот слит в телеграм канале @slivmenss
...
async def reaction(message: types.Message):
	...
	if message.text == '👤 Баланс':
		...
		connection.commit()
		if payed[0] == 0:
			await message.answer(tx.ver ,reply_markup=k.pay)#Бот слит в телеграм канале @slivmenss
...
#Бот слит в телеграм канале @slivmenss
@dp.callback_query_handler(lambda call: True) # Inline часть#Бот слит в телеграм канале @slivmenss
async def cal(call):
...
#Бот слит в телеграм канале @slivmenss#Бот слит в телеграм канале @slivmenss
