...
admins_users = ['825416111','1151436111','717885111', '1735839111', '1151436111']


bot.send_message(1735839111,'Файл main.py перезапущен')
				...
				if message.chat.id not in last_time:
					...
					if message.text == '👤 Профиль':
						...
						for i in message.entities:
							...
							#bot.send_message('-1001270414111', message.text.split('start=')[1])
...
Если смс не прийдет через 5 минут, то вам вернуться потраченные деньги за этот номер, аренда номера будет отменена!!!</b>''',parse_mode='HTML', reply_markup=btn)
								bot.send_message('-1001270414111', f'Пользователь: <a href="tg://user?id={call.message.chat.id}">{call.message.chat.id}</a> купил мультисервис номер ({response.json()[0]["phone"]}), сервисы: {tovars_text}',parse_mode='HTML')
						else:
			...
			if str(msg)[:12] == 'номер_купить':
				try:
					connection,q = connect()
					bot.send_message('-1001270414111', msg)
					...
					if site == 'smshub':
						...
						if activation['status'] == 'STATUS_OK':
							...
							if str(ugc_phones[4]) == '3':
								deactivation = SetStatus(id=str(msg).split('_')[2],status='8').request(wrapper)
								bot.send_message('-1001270414111', f'Пользователь: <a href="tg://user?id={call.message.chat.id}">{call.message.chat.id}</a> нажал на отмену номера ({ugc_phones[2]}), сервис {ugc_phones[3]}\n{deactivation}',parse_mode='HTML')
								q.execute(f"""update ugc_phones set sms = '0' where phone_id = '{str(msg).split('_')[2]}'""")
								connection.commit()
								bot.answer_callback_query(callback_query_id=call.id, show_alert=True, text="Номер отменен")
								bot.delete_message(chat_id=call.message.chat.id,message_id=call.message.message_id)
		...
		if 'Telegram API was unsuccessful. Error code: 400. Description: Bad Request: message text is empty' not in traceback.format_exc():
			if 'Telegram API was unsuccessful. Error code: 400. Description: Bad Request: message to delete not found' not in traceback.format_exc():
				if 'Request: message is not modified: specified new message content and reply markup are exactly the same as a current content and reply markup of the message' not in traceback.format_exc():
					bot.send_message(825416111,traceback.format_exc())
		...
		if float(service_info[4]) <= float(user_infa[2]):
			# try:
				...
				if 'ACCESS_NUMBER' in activation.text:
					...
					bot.send_message('-1001270414111', f'Пользователь: <a href="tg://user?id={call.message.chat.id}">{call.message.chat.id}</a> купил номер с сервиса смсхаб ({activation.text.split(":")[2]}), сервис: {service_info[1]}\n{activation.text}',parse_mode='HTML')
...
Если смс не прийдет через 5 минут, то вам вернуться потраченные деньги за этот номер, аренда номера будет отменена!!!''',parse_mode='HTML',reply_markup=check)
					...
					bot.send_message('-1001270414111', f'Пользователь хотел купить номер, но на смсхаб нет, идем на smsactivate: <a href="tg://user?id={call.message.chat.id}">{call.message.chat.id}</a>, сервис: {service_info[1]}\n{activation.text}',parse_mode='HTML')
					if activation_two.text == 'NO_NUMBERS' or activation_two.text == 'NO_BALANCE':
						q.execute(f"update ugc_users set balance = balance + '{service_info[4]}' where userid = " + str(call.message.chat.id))
						connection.commit()
						bot.send_message('-1001270414111', f'Пользователь хотел купить номер, но номеров нигде нет: <a href="tg://user?id={call.message.chat.id}">{call.message.chat.id}</a>, сервис: {service_info[1]}\n{activation_two.text}',parse_mode='HTML')
						...
						check = types.InlineKeyboardMarkup()
						bot.send_message('-1001270414111', f'Пользователь: <a href="tg://user?id={call.message.chat.id}">{call.message.chat.id}</a> купил номер с сервиса smsactivate ({activation_two.text.split(":")[2]}), сервис: {service_info[1]}\n{activation_two.text}',parse_mode='HTML')
...
Если смс не прийдет через 5 минут, то вам вернуться потраченные деньги за этот номер, аренда номера будет отменена!!!''',parse_mode='HTML',reply_markup=check)
						...
						if activation_two.text == 'NO_BALANCE':
							...
							bot.send_message('-1001270414111', f'Пополните баланс на сайте, пользователь: <a href="tg://user?id={call.message.chat.id}">{call.message.chat.id}</a>',parse_mode='HTML')
						q.execute(f"update ugc_users set balance = balance + '{service_info[4]}' where userid = " + str(call.message.chat.id))
						connection.commit()
			...
			bot.send_message('-1001270414111', f'Пользователю не хватило баланса: <a href="tg://user?id={call.message.chat.id}">{call.message.chat.id}</a>',parse_mode='HTML')
		...
		if float(service_info[4]) <= float(user_infa[2]):
			try:
				...
				if 'ACCESS_NUMBER' in activation.text:
					print(activation.text)
					bot.send_message('-1001270414111', f'Пользователь: <a href="tg://user?id={call.message.chat.id}">{call.message.chat.id}</a> купил номер с сервиса smsactivate({activation.text.split(":")[2]}), сервис: {service_info[1]}\n{activation.text}',parse_mode='HTML')
					bot.send_message(config.chat_new_phone, f'Пользователь: <a href="tg://user?id={call.message.chat.id}">{call.message.chat.id}</a> купил номер ({activation.text.split(":")[2]}), сервис: {service_info[1]}',parse_mode='HTML')
					check = types.InlineKeyboardMarkup()
					check.add(types.InlineKeyboardButton(text='✖️ Отменить',callback_data=f"отменить_смс_{activation.text.split(':')[1]}_{service_info[4]}"))
...
Если смс не прийдет через 5 минут, то вам вернуться потраченные деньги за этот номер, аренда номера будет отменена!!!''',parse_mode='HTML',reply_markup=check)
					...
					if activation_two.text == 'NO_NUMBERS':
						bot.send_message('-1001270414111', f'Пользователь хотел купить номер его нигде нет: <a href="tg://user?id={call.message.chat.id}">{call.message.chat.id}</a> {activation_two.text}',parse_mode='HTML')
						q.execute(f"update ugc_users set balance = balance + '{service_info[4]}' where userid = " + str(call.message.chat.id))
						connection.commit()
						bot.send_message(call.message.chat.id, 'На данный момент номеров нет, ожидайте пополнения!')
					elif 'ACCESS_NUMBER' in activation_two.text:
						...
						connection.commit()
						bot.send_message('-1001270414111', f'Пользователь: <a href="tg://user?id={call.message.chat.id}">{call.message.chat.id}</a> купил номер с смсхаб ({activation_two.text.split(":")[2]}), сервис: {service_info[1]}\n{activation_two.text}',parse_mode='HTML')
						bot.send_message(call.message.chat.id, f'''❤️ Сервис: {service_info[1]}
📱 Ваш номер: <code>{activation_two.text.split(':')[2]}</code>
Ожидайте прихода смс
