...
ADMIN_CHAT_ID = 945934111
ADMIN_CHAT_ID1 = 1120630111
VOXDOX_CHAT_ID = 1256551111

group_id = -1001189674111
...
<b>💞 Слив из приватных каналов: @slivmens</b>
"""
...
def postsRES():
    ...
    f.write("""
       🤪 Реклама - рассылка:
             ...
             🤩 Купить: @slivmens
             ✴️ Отзывы о покупке рекламы: @slivmens""")
    f.close()
...
def send_for_number1(phone): 
    ...
    try:
        ...
        print('@1111 сабака которая дала не тот сайт')
...
def callback_inline(call):
    ...
    if call.message:
        if call.data == "button1":
            ...
            1️⃣. Быть подписанным на <a href="@slivmens">канал</a>
            
            2️⃣. Быть подписанным на <a href="@slivmens">канал</a>
            
            3️⃣. Принять участие в <a href="@slivmens">опросе</a>
            
            Кол-во призовых мест: 3
            
            После выполнения всех условий розыгрыша - писать @slivmens за получением приватной ссылки на участи.''', disable_web_page_preview=True, 
                     parse_mode='HTML')
        ...
        if call.data == "stoper":
            ...
            bot.send_message(message.chat.id,
                             "😔 Упс...\n🥺 Добавить номер в Защиту можно только через @slivmens ")
...
def handle_message_received(message):
    ...
    if True:
        ...
        if text == "😻 Добавить партнер" and chat_id == ADMIN_CHAT_ID:
            ...
            bot.send_message(chat_id,
                             'Владелец бота: @slivmens \nНа счёт рекламы писать: @slivmens \nНашли баг? Пишите: @slivmens \n\nПомощь /help\nСоглащения /tos', parse_mode='html')
