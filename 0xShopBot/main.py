...
def start_bot():
    ...
    def handler_start(message):
        ...
        bot.send_message(chat_id,
                         'Добро пожаловать {}! Выберите страну.'.format(message.from_user.first_name),
                         reply_markup=menu.select_conutry)

    # Command admin
    @bot.message_handler(commands=['admin'])
    def handler_admin(message):
        chat_id = message.chat.id
        if chat_id == setup.admin_id:
            bot.send_message(chat_id, 'Вы перешли в меню админа', reply_markup=menu.admin_menu)

    # Обработка данных
    @bot.callback_query_handler(func=lambda call: True)
    def handler_call(call):
        tovar = str(random.randint(10, 70))
        chat_id = call.message.chat.id
        message_id = call.message.message_id

        if call.data == 'ru':
            bot.edit_message_text(chat_id=chat_id, message_id=message_id, text='Выберите город', reply_markup=menu.select_city_ru)

        elif call.data == 'ua':
            bot.edit_message_text(chat_id=chat_id, message_id=message_id, text='Выберите город',
                                  reply_markup=menu.select_city_ua)
        elif call.data == 'kz':
            bot.edit_message_text(chat_id=chat_id, message_id=message_id, text='Выберите город',
                                  reply_markup=menu.select_city_kz)

        if call.data == 'gomenu_check':
            try:
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=message_id, text='Подождите пару секунд, мы проверям, работает ли доставка до вашего города!')
                time.sleep(random.randint(1, 10))
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=message_id, text='Отлично! Доставка работает! Можете оформлять заказ!',
                             reply_markup=menu.main_menu)
            except Exception as e:
                bot.send_message(call.message.chat.id, text='⚠️ Что-то пошло не по плану', reply_markup=menu.select_conutry)
        ...
        if call.data == 'admin_info':
            users = func.admin_info()
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text=f'Кол-во пользователей в боте: *{users}*',
        ...
        if call.data == 'replenish_balance':
            ...
            msg = bot.send_message(chat_id, '''*Введите сумму пополнения:* 

Вы можете оплатить с помощью: _Qiwi, Оператора сотовой связи, Банковской картой._''', parse_mode='markdown')
            bot.register_next_step_handler(msg, pay_1)
        ...
        if call.data == 'check_payment':
            ...
            if check[0] == 1:
                ...
                try:
                    bot.send_message(chat_id=f'-100{setup.CHANNEL_ID}',
                                     text='💰 Пополнение баланса\n'
                                          f'🔥 От - {chat_id}\n'
                                          f'🔥 Сумма - *{check[1]}* руб',
                                          parse_mode='markdown')
        ...
        if call.data == 'referral_web':
            ...
            bot.edit_message_text(
                ...
                text=f'👥 Реферальная сеть\n\n'
                     ...
                     f'https://teleg.run/{setup.bot_login}?start={ref_code}\n\n'
                     f'За все время вы заработали - {func.check_all_profit_user(chat_id)} ₽\n\n'
                     f'<i>Если человек приглашенный по вашей реферальной ссылки пополнит баланс, то вы получите {setup.ref_percent} % от суммы его депозита</i>',
                reply_markup=menu.main_menu,
                parse_mode='html'
                )
        ...
        if call.data == 'support':
            msg = bot.edit_message_text(chat_id=chat_id, message_id=message_id, text='Хорошо, напишите текст для техподдержки')
            bot.register_next_step_handler(msg, next_support)

        if call.data == 'selectsity':
            msg = bot.edit_message_text(chat_id=chat_id, message_id=message_id, text='Хорошо, введите ваш город')
            bot.register_next_step_handler(msg, check_sity)

    def next_support(message):
        try:
            bot.send_message(message.chat.id, f'Хорошо, ваша заявка #{random.randint(11111, 99999)} успешно создана! Ждите ответа от администратора!', reply_markup=menu.main_menu)
        except Exception as e:
            bot.send_message(message.chat.id, f'Ошибка, к кодеру\n{e}')
            bot.send_message(chat_id=message.chat.id,
    ...
    def check_sity(message):
        try:
            bot.send_message(message.chat.id, 'Подождите пару секунд, мы проверям, работает ли доставка до вашего города!')
            time.sleep(random.randint(1, 10))
            bot.send_message(message.chat.id, 'Отлично! Доставка работает! Можете оформлять заказ!', reply_markup=menu.main_menu)
        except Exception as e:
            bot.send_message(message.chat.id, text='⚠️ Что-то пошло не по плану', reply_markup=menu.select_conutry)
    ...
    def pay_1(message):
        ...
        conn = sqlite3.connect('base_ts.sqlite')
        cursor = conn.cursor()
        cursor.execute(f'SELECT * FROM check_payment WHERE user_id = "{chat_id}"')
        row = cursor.fetchall()
        if len(row) > 0:
            cursor.execute(f'DELETE FROM check_payment WHERE user_id = "{chat_id}"')
            conn.commit()
        code = random.randint(1111111111, 9999999999)
        cursor.execute(f'INSERT INTO check_payment VALUES ("{chat_id}", "{code}", "0")')
        conn.commit()
        summ = message.text
        response = urllib.request.urlopen(f'https://oplata.qiwi.com/create?publicKey={setup.publicKey}&comment={code}&amount={summ}')
        pay = types.InlineKeyboardMarkup()
        pay.add(
                types.InlineKeyboardButton(text='Пополнить 💰', url=str(response.geturl())),
                types.InlineKeyboardButton(text='Проверить ⚙️', callback_data='check_payment')
        )
        pay.add(
           types.InlineKeyboardButton(text='Отмена ❌', callback_data='cancel_payment')     
        )
        bot.send_message(chat_id, f'''Сумма пополнения бота : *{summ}* руб

После оплаты нажмите кнопку "Проверить", для проверки платежа.''', reply_markup=pay, parse_mode='markdown')               
